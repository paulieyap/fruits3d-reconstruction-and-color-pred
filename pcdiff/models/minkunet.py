...
class MinkGlobalEnc(nn.Module):
    def __init__(self, **kwargs):
        ...
        in_channels = kwargs.get('in_channels', 6) #3
        cs = [32, 32, 64, 128, 256, 256, 128, 96, 96]
        cs = [int(cr * x) for x in cs]
        self.embed_dim = cs[-1]
        self.run_up = kwargs.get('run_up', True)
        self.D = kwargs.get('D', 3) #this is just the coords dimension - used to get the neighbors 
...
class MinkUNetDiff(nn.Module):
    def __init__(self, **kwargs):
        ...
        #self.latent_enc = LatentTransformer(d_model=cs[4])
        ...
        self.last  = nn.Sequential(
            nn.Linear(cs[8], 20),
            nn.LeakyReLU(0.1, inplace=True),
            nn.Linear(20, out_channels), #change to 6 output size 
        )
    ...
    def match_part_to_full(self, x_full, x_part):
        #########################################################################################################
        # we match points from the partial pcd to the full pcd                                                  #
        # note that we multiply the batch id by 10000 so only points on the same batch sample will be matched   #
        #########################################################################################################
        full_c = x_full.C.clone().float()
        part_c = x_part.C.clone().float()

        bidx_f = full_c[:,0][:,None]
        bidx_p = part_c[:,0][:,None]
        bidx_match = ~(bidx_f == bidx_p.T)

        dist_coord = torch.cdist(full_c[:,1:], part_c[:,1:])
        dist_coord[bidx_match] = np.inf
        match_feats = torch.argmin(dist_coord, axis=1)
